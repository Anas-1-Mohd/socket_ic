 ...
 type AppMessage = {
  ...
  func send_app_message(client_principal : IcWebSocketCdk.ClientPrincipal, msg : AppMessage): async () {
    ...
    switch (await IcWebSocketCdk.send(ws_state, client_principal, to_candid(msg))) {
      ...
      case (_) { Debug.print("Successfully Sent"); };
    };
  };
  ...
  func on_open(args : IcWebSocketCdk.OnOpenCallbackArgs) : async () {
    ...
    Debug.print("args.client_principal: " # debug_show (args.client_principal));
  ...
  public shared ({ caller }) func ws_open(args : IcWebSocketCdk.CanisterWsOpenArguments) : async IcWebSocketCdk.CanisterWsOpenResult {
    let caller : Principal = Principal.fromText("gelwp-gaysy-pgvzl-h3772-nwy6h-iof3r-5ld2o-hvd7r-6cfg4-i5jnb-bqe");
